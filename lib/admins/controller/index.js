...
const getUserLinks = async (req, res, next) => {
  try {
    ...
    if (result) {
      return responses.successWithMessage("User links received successfully", res, result);
    }
...
const deleteLink = async (req, res, next) => {
  try {
    ...
    if (!link) return responses.failedWithMessage("Link does not exist", res);
...
const addLinkType = async (req, res, next) => {
  try {
    ...
    return responses.successWithMessage(`Link type added successfully `, res, result);
  } catch (err) {
    console.log(err);
    return responses.serverError(res);
  }
...
const editLinkType = async (req, res, next) => {
  try {
    ...
    if (result.updated) {
      return responses.successWithMessage("Link type edited successfully", res, result.linktype);
    }
...
const editLinkIcon = async (req, res, next) => {
  try {
    const icon = req?.file;
    const linkTypeId = req.params.id;
    ...
    await uploadBytes(imageRef, icon?.buffer, metaType).then(async () => {
      const publicUrl = await getDownloadURL(imageRef);
      const result = await service.editLinkIcon(linkTypeId, publicUrl);
      if (!result.updated) return responses.failedWithMessage("Failed to upload the icon of  link", res);

      return responses.successWithMessage(`Link icon updated successfully`, res, result.linktype);
    }).catch((e) => {
      console.error(e);
