...
const addLinkType = async (req, res, next) => {
  try {
    ...
    await uploadBytes(imageRef, file?.buffer, metaType).then(async () => {
      ...
      const result = await service.addLinkType(type, publicUrl);
      console.log("result", result)
      if (!result) return responses.failedWithMessage("Failed to uplaod the icon of  link", res);
      return responses.successWithMessage(`the link add successfully `, res, { urlImage: publicUrl});
    }).catch((e) => {
      console.error(e);
...
const getLinkTypes = async (req, res, next) => {
  try {
    ...
    if(linktypes) return responses.success("successfully get linkTypes", linktypes ,res)
