...
const getLinkByQR = async (req, res) => {
  try {
    ...
    if (!qrCodeId) return responses.failedWithMessage("faild to scan qr", res);
    ...
    if (links){
      return responses.successWithMessage(
        "successflly get links",
        res, 
        transformers.linksTransformers(links)
      );
    }
    return responses.failedWithMessage("failed to get links code ! ", res);
  } catch (err) {
